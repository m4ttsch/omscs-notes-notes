...
When all switches have received rules corresponding to the `p2`, incoming packets can start being tagged with `p2`.
...
Various people may write these applications, such as network operators, service provides and researchers.
...
Another feature of pyretic is the notion of boolean predicates. Unlikes OpenFlow rules, which do not permit the use of simple conjunctions, like `and` or `or`, or negations, like `not`, Pyretic allows the expression of policies in terms of these predicates.
...
They are represented as time series of static policies, and the current value of the policy at any time is `self.policy`.
