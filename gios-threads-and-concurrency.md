---
id: gios-threads-and-concurrency
...
After the fork completes, the process now have two threads both of which can execute concurrently.
...
One naive approach would be to wrap access to the share state itself in the mutex. However, this approach is too restrictive. Since mutexes only allow access to shared state one thread at time, we would be able to let multiple readers access state concurrently.
...
This is where the problem lies. T2 will not be able to lock the mutex for A, because T1 is holding it. T1 will not be able to lock the mutex for B, because T1 is holding it. _More importantly_, neither T1 nor T2 will be able to release the mutexes that the other needs, since they are both blocking trying to acquire the mutex the other has.
